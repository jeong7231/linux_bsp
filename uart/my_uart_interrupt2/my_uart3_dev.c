...
/* 공유 IRQ 가능. dmesg로 확인한 번호를 모듈 파라미터로 전달 */
static int irq = -1;
module_param(irq, int, 0444);
MODULE_PARM_DESC(irq, "UART3 IRQ number (dmesg의 fe201600.serial irq)");
...
static irqreturn_t my_uart3_isr(int irqno, void *dev_id)
{
    ...
    if (mis & (UART_IMSC_RXIM | UART_IMSC_RTIM)) {
        ...
        while (!(readl(uart3_base + UART_FR) & UART_FR_RXFE)) {
            ...
            if (!rb_full(&rxrb)) rb_put(&rxrb, c); /* overflow 시 드롭 */
        }
...
static int my_uart3_open(struct inode *inode, struct file *file)
{
    ...
    /* FIFO trigger level: RX 1/2, TX 1/2 */
    writel((0x2 << 3) | (0x2 << 0), uart3_base + UART_IFLS);

    /* Mask: RX, RT. TX는 필요시 enable */
...
static ssize_t my_uart3_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        /* 여유 없으면 즉시 전송 시도 후 대기 없이 드롭 방지 위해 sleep 옵션 없앰. 필요시 대기 추가 가능 */
        spin_lock_irqsave(&txrb.lock, flags);
        if (rb_full(&txrb)) {
            spin_unlock_irqrestore(&txrb.lock, flags);
            /* FIFO로 직접 밀어넣기 시도 */
            ...
            /* 링버퍼 여유 생길 때까지 인터럽트가 처리. 사용자 사이드에서 반복 호출 가정 */
            break;
        } else {
            rb_put(&txrb, ch);
            spin_unlock_irqrestore(&txrb.lock, flags);
...
/* read: 링버퍼에서 비동기 읽기. 데이터 없으면 0 반환(논블로킹) */
...
{
    if (irq < 0) {
        pr_err("my_uart3: set module param irq=<number> (see dmesg fe201600.serial)\n");
        return -EINVAL;
    }
